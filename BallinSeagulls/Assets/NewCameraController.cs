...
public class NewCameraController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (player.currentState == NewPlayerController.State.NORMAL)
        {
    ...
    void LateUpdate()
    {
        switch (player.currentState) {
            case NewPlayerController.State.WAIT:
                SpiralProcedure();
                break;
            case NewPlayerController.State.NORMAL:
                ...
                FollowTarget();
                break;
            case NewPlayerController.State.DEAD:
    ...
    void StartStage()
    {
        player.currentState = NewPlayerController.State.NORMAL;
    }
    ...
    void FollowTarget()
    {
        ...
        Vector3 vectorA = new Vector3(transform.forward.x, 20.0f, transform.forward.z);

        // Get target's velocity vector minus the y component
        Vector3 vectorB = new Vector3(player.rigidBody.velocity.x, 20.0f, player.rigidBody.velocity.z);
